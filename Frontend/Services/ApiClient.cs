...
public class ApiClient
{
    ...
    {
        return await _http.GetFromJsonAsync<List<DomestiqueResponse>>("api/domestique") ?? [];
    }

    public async Task<bool> SaveDomestiqueAsync(string person, string activite, decimal heuresParSemaine, decimal inseeRefFemme, decimal inseeRefHomme)
    {
        var response = await _http.PostAsJsonAsync("api/domestique", new
        {
            person,
            activite,
            heuresParSemaine,
    ...
    {
        var response = await _http.DeleteAsync($"api/domestique/{id}");
        return response.IsSuccessStatusCode;
    }
}
